class DropBoxController {
    ...
    connectFirebase() { //para inicializar o firebase

        var firebaseConfig = {
            apiKey: "AIzaSyA6-f7VLWr7GSc9d39hrJmN4ZuWkdsA1Ck",
            authDomain: "dropbox-clone-741df.firebaseapp.com",
            databaseURL: "https://dropbox-clone-741df.firebaseio.com",
            projectId: "dropbox-clone-741df",
            storageBucket: "dropbox-clone-741df.appspot.com",
            messagingSenderId: "606750364578",
            appId: "1:606750364578:web:2a2e8c82dc54d996b158fb",
            measurementId: "G-EQKKZF92BQ"
        };
        // Initialize Firebase
    ...
    uploadComplete() { //finaliza o upload e reseta a página

        this.modalShow(false);  //esconde a barra de envio
        this.inputFilesEl.value = '';   //reseta o arquivo a ser enviado
        this.btnSendFileEl.disabled = false; //habilita o borão de enviar arquivo
    ...
    modalShow(show = true) { //para mostrar ou não a barra de carregamento de arquivos

        this.snackModalEl.style.display = (show) ? 'block' : 'none'; //para dar display na barra de carregamento
        ...
        [...files].forEach(file => { //para cada arquivo cria uma promessa com envio ajax para o db
            ...
            promises.push(new Promise((resolve, reject) => {
                ...
                task.on('state_changed', snapshot => { //quase como um forEach
                    ...
                    fileRef.getMetadata().then(metadata => { //para pegar os dados do arquivo e devolver como promessa
    ...
    uploadProgress(event, file) { //para atualizar a barra de envio enquanto acontece o upload
    ...
    } //fechando o formatTimeToHuman()

    getFileIconView(file) { //para tratar o ícone do arquivo
    ...
    } //fechando o getFileIconView()

    getFileView(file, key) { //para mostrar o arquivo com seu tipo na tela 
    ...
    readFiles() { //para ler os arquivos do firebase
        ...
        this.getFirebaseRef().on('value', snapshot => { //para carregar as referências do db num determinado momento
            ...
            snapshot.forEach(snapshotItem => { //para cada item
        ...
        li.addEventListener('dblclick', e => {
            ...
            switch (file.type) {
                ...
                case 'folder':
                    ...
                    break;
                default:
                    window.open(file.path); //para abrir o arquivo
